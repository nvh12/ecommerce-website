...
const MousePage = () => {
  ...
  useEffect(() => {
    const fetchMouses = async () => {
      try {
        ...
        setMouseProducts(response.data.product || []);
        setLoading(false);
      } catch (err) {
        console.error('Error fetching Mouses:', err);
        setError('Không có Chuột nào');
        setLoading(false);
      }
    };
  ...
  const brands = Array.from(new Set(MouseProducts.map((p) => p.brand).filter(Boolean)));
    ...
    <div style={{ background: '#f8f9fa', minHeight: '100vh' }}>
      <Container className="py-4">
        ...
        <div className="d-flex flex-column flex-md-row align-items-md-center justify-content-between mb-4">
          <h2 className="mb-3 mb-md-0 fw-bold text-primary">Chuột </h2>
        ...
        {/* Phân trang */}
        <div className="mt-4">
          <Pagination pageName="productsPage" category='Mouse' brand={brand} setItems={setMouseProducts} />
        </div>
      </Container>
    </div>
  );
