...
const WatchPage = () => {
  ...
  useEffect(() => {
    const fetchWatches = async () => {
      try {
        ...
        setWatchProducts(response.data.product || []);
        setLoading(false);
      } catch (err) {
  ...
  const brands = Array.from(new Set(watchProducts.map((p) => p.brand).filter(Boolean)));
    ...
    <div style={{ background: '#f8f9fa', minHeight: '100vh' }}>
      <Container className="py-4">
        <div className="d-flex flex-column flex-md-row align-items-md-center justify-content-between mb-4">
          <h2 className="mb-3 mb-md-0 fw-bold text-primary">Đồng Hồ</h2>
        ...
        <div className="mt-4">
          <Pagination pageName="productsPage" category='watch' brand={brand} setItems={setWatchProducts} />
        </div>
      </Container>
    </div>
  );
